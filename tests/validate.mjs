...
for (const marker of ["debts", "investments", "patrimony", "validateDebts_", "validatePatrimony_", "validateInvestmentOperations_", "validateInvestmentCorrections_", "validateTransactions_", "investmentByOperation", "transactionCount", "operations", "corrections"]) assert.match(backend, new RegExp(marker), `coleção ausente no backend: ${marker}`);
for (const marker of ["cards", "cardPayments", "validateCards_", "cardPaymentId", "transactionId", "accountId"]) assert.match(backend, new RegExp(marker), `contrato de cartões ausente no backend: ${marker}`);
...
assert.match(js, /const controls = editorForm\.elements/, "o editor de foto precisa usar os controles do formulário");
assert.doesNotMatch(js, /dialog\.elements\.zoom/, "o editor não pode ler controles no elemento dialog");
...
assert.match(backend, /validateCards_\(payload\.cards \|\| \[\], payload\.cardPayments \|\| \[\], payload\.transactions \|\| \[\], payload\.accounts \|\| \[\]\)/i, "o backend precisa validar cartões, faturas e saídas vinculadas");
assert.match(js, /validateCardsPayload\(vault\.cards, vault\.cardPayments, vault\.transactions, vault\.accounts\)/i, "o frontend precisa validar cartões antes de sincronizar");
assert.match(js, /cardPaymentId: paymentId/, "o frontend precisa vincular a fatura paga ao lançamento de saída");
assert.match(js, /type: "saida"/, "a fatura paga precisa gerar uma saída");
